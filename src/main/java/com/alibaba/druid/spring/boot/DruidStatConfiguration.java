...
/**
 * 
 * @className ： DruidStatConfiguration
...
@ConditionalOnProperty(
	name = { "spring.datasource.druid.stat.enable" }, 
	havingValue = "true", 
...
public class DruidStatConfiguration {
	 ...
	 * 提供监控信息展示的html页面 - 提供监控信息的JSON API 注意：使用StatViewServlet，建议使用druid
	 * 0.2.6以上版本。
	 */
	@Bean
	public ServletRegistrationBean druidServletRegistrationBean(DruidStatProperties statProperties) {
		...
		ServletRegistrationBean servletRegistrationBean = new ServletRegistrationBean(new StatViewServlet(), statProperties.getServletPath());
		// 添加初始化参数：initParams
		
		// 是否能够重置数据(禁用HTML页面上的“Reset All”功能)
		servletRegistrationBean.addInitParameter(StatViewServlet.PARAM_NAME_RESET_ENABLE, statProperties.getResetEnable().toString());
				
		// 用户对象在Session中存储值所用的Key		
		if (StringUtils.isNotEmpty(statProperties.getSessionUserKey())) {
			servletRegistrationBean.addInitParameter(StatViewServlet.SESSION_USER_KEY, statProperties.getSessionUserKey());
		}
		
		// 登录查看信息的账号密码.
		if (StringUtils.isNotEmpty(statProperties.getLoginUsername())
				&& StringUtils.isNotEmpty(statProperties.getLoginPassword())) {
			servletRegistrationBean.addInitParameter(StatViewServlet.PARAM_NAME_USERNAME, statProperties.getLoginUsername());
			servletRegistrationBean.addInitParameter(StatViewServlet.PARAM_NAME_PASSWORD, statProperties.getLoginPassword());
		}
		...
		servletRegistrationBean.addInitParameter(StatViewServlet.PARAM_NAME_JMX_USERNAME, statProperties.getJmxUsername());
		servletRegistrationBean.addInitParameter(StatViewServlet.PARAM_NAME_JMX_PASSWORD, statProperties.getJmxPassword());
	...
	public FilterRegistrationBean duridFilterRegistrationBean(DruidStatProperties statProperties) {
		...
		// Session监控配置
		filterRegistrationBean.addInitParameter(WebStatFilter.PARAM_NAME_PROFILE_ENABLE, statProperties.getProfileEnable().toString());
		filterRegistrationBean.addInitParameter(WebStatFilter.PARAM_NAME_SESSION_STAT_ENABLE, statProperties.getSessionStatEnable().toString());
		filterRegistrationBean.addInitParameter(WebStatFilter.PARAM_NAME_SESSION_STAT_MAX_COUNT, statProperties.getSessionStatMaxCount().toString());
		...
		filterRegistrationBean.addInitParameter(WebStatFilter.PARAM_NAME_PRINCIPAL_SESSION_NAME, statProperties.getPrincipalSessionName());
		filterRegistrationBean.addInitParameter(WebStatFilter.PARAM_NAME_PRINCIPAL_COOKIE_NAME, statProperties.getPrincipalCookieName());
		filterRegistrationBean.addInitParameter(WebStatFilter.PARAM_NAME_REAL_IP_HEADER, statProperties.getRealIpHeader());
